...
‘’’Juste the full code 2 avec tous ce comment le code 1 gere l’envois du voice ou texte et aussi uploads puis envois le lien’’’

Je veux tous ce code la barre de texte code 1 envois ect les message sont gere dans le code 2 je dit bien tous et uniquememt la barre de texte + send/voice envoi ect tous la facon que 1 le gere dans le code 2
...
.scroll-container {
    ...
    /* Correction Android : On utilise Flexbox au lieu de height 100% */
    flex: 1;
    min-height: 0;
...
<body>
    ...
    <script>
        const API = "https://server.adamdh7.org";
    ...
    <script>
        ...
        function createMessageElement(m) {
            ...
            if (text.startsWith(REPLY_START)) {
                ...
                const mainSpan = document.createElement('span');
                mainSpan.className = 'main-message';
                mainSpan.innerHTML = renderText(main, isMe);
                div.appendChild(mainSpan);
            } else {
        ...
        function renderReplyPreview(quoted) {
            if (quoted.startsWith(PUBLIC_URL)) {
                ...
                if (['jpeg','jpg','gif','png','heic','webp','avif','bmp','tiff','svg'].includes(ext)) {
                    ...
                    preview = `<i class="fa-solid fa-video reply-preview-icon"></i> <span>${translations[currentLanguage]["video-message"]}</span>`;
                } else if (['mp3','wav','ogg','aac','m4a','flac'].includes(ext)) {
                    preview = `<i class="fa-solid fa-microphone reply-preview-icon"></i> <span>${translations[currentLanguage]["voice-message"]}</span>`;
                } else {
                    preview = `<i class="fa-solid fa-file reply-preview-icon"></i> <span>${translations[currentLanguage]["file-message"]}</span>`;
                }
                return preview;
            } else {
                const lines = quoted.split('\n');
        ...
        function updateCheckmarks(messages) {
            ...
            const bubbles = container.querySelectorAll('.bubble.me');
            bubbles.forEach((bubble, idx) => {
                const msgTime = bubble.id.replace('msg-', '').replace(/-/g, ':');
                const msgIndex = messages.findIndex(m => m.time === msgTime && m.from === user.tfid);
                if (msgIndex === -1) return;
                const m = messages[msgIndex];
                let read = m.read;
        ...
        function getPreviewText(text) {
            const main = getMainMessage(text);
            if (main.startsWith(PUBLIC_URL)) {
                const filename = main.split('/').pop();
                const ext = filename.split('.').pop().toLowerCase();
                if (['jpeg','jpg','gif','png','heic','webp','avif','bmp','tiff','svg'].includes(ext)) {
                    return translations[currentLanguage]["image-message"];
                } else if (['mp4','mov','avi','mkv','hevc','webm','ogv','3gp'].includes(ext)) {
                    return translations[currentLanguage]["video-message"];
                } else if (['mp3','wav','ogg','aac','m4a','flac'].includes(ext)) {
                    return translations[currentLanguage]["voice-message"];
                } else {
        ...
        function addMessageListeners(bubble, text) {
            addLongPressListeners(bubble, (x, y) => {
                if (isTextMessage(text)) {
                    showCopyMenu(x, y, text, bubble);
                }
            });
            let touchStartX = 0;
            bubble.addEventListener('touchstart', (e) => {
        ...
        function isTextMessage(text) {
            return !text.startsWith(PUBLIC_URL) && !text.startsWith('https://url.adamdh7.org/');
        }

        // Fonction unique pour envoyer tous les messages (texte, média, voix, reply)
        async function sendMessage(content) {
            if (!content || !activePeer) return;

            try {
                const response = await fetch(`https://server.adamdh7.org/send`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        sender_tfid: user.tfid,
                ...
                alert("Mesaj pa ale. Verifiez console pou plis detay.");
            }
        }

        // Envoi texte avec gestion reply
        ...
        function replyToMessage(quoted, sender, messageId) {
            ...
            if (quoted.startsWith(PUBLIC_URL)) {
                ...
                if (['jpeg','jpg','gif','png','heic','webp','avif','bmp','tiff','svg'].includes(ext)) {
                    ...
                    mediaPreview.innerHTML = `<i class="fa-solid fa-video reply-preview-icon"></i> <span>${translations[currentLanguage]["video-message"]}</span>`;
                } else if (['mp3','wav','ogg','aac','m4a','flac'].includes(ext)) {
                    mediaPreview.innerHTML = `<i class="fa-solid fa-microphone reply-preview-icon"></i> <span>${translations[currentLanguage]["voice-message"]}</span>`;
                } else {
                    mediaPreview.innerHTML = `<i class="fa-solid fa-file reply-preview-icon"></i> <span>${translations[currentLanguage]["file-message"]}</span>`;
                }
            } else {
                replyText.textContent = quoted.substring(0, 50) + (quoted.length > 50 ? '…' : '');
            }
        ...
        async function showContactPreview(peer) {
            ...
            try {
                ...
                last7.forEach(m => {
                    ...
                    const div = document.createElement('div');
                    div.className = `mini-bubble ${isMe ? 'me' : 'them'}`;
                    div.innerText = getPreviewText(getMainMessage(m.text));
                    msgDiv.appendChild(div);
                });
            } catch (e) {
                console.error(e);
        ...
        function renderText(text, isMe) {
            if (text.startsWith(PUBLIC_URL) || text.startsWith('https://url.adamdh7.org/')) {
                const filename = text.split('/').pop();
                const ext = filename.split('.').pop().toLowerCase();
                if (['jpeg','jpg','gif','png','heic','webp','avif','bmp','tiff','svg'].includes(ext)) {
                    return `<div class="media-content" onclick="openFullscreen('${text}', 'image')"><img src="${text}" loading="lazy" alt="${filename}" /></div>`;
                } else if (['mp4','mov','avi','mkv','hevc','webm','ogv','3gp'].includes(ext)) {
                    return `<div class="media-content" onclick="openFullscreen('${text}', 'video')"><video src="${text}" playsinline muted preload="metadata" poster="${text}#t=0.1"></video></div>`;
                } else if (['mp3','wav','ogg','aac','m4a','flac'].includes(ext)) {
                    return renderCustomVoicePlayer(text);
                } else {
                    return `<div class="file-preview" onclick="openFileInNewTab('${text}')"><i class="fa-solid fa-file file-icon"></i><span class="file-link">${filename}</span></div>`;
            ...
            let processedText = text.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" class="underline text-blue-300">$1</a>');
            processedText = processedText.replace(/(TF-\w+)|(\w+@dh7\.tf)/g, '<span class="text-blue-300 cursor-pointer" onclick="openChatByIdentifier(\'$1$2\')">$1$2</span>');

            return truncateText(processedText);
        }

        function truncateText(text) {
            const CHUNK_SIZE = 700;
            if (text.length <= CHUNK_SIZE) return `<span class="message-text">${text}</span>`;
            let html = '';
            let remaining = text;
            let chunkId = 0;
            while (remaining.length > 0) {
                ...
                html += `<span class="message-text chunk-${chunkId}" ${chunkId > 0 ? 'style="display:none;"' : ''}>${chunk}</span>`;
                if (remaining.length > 0) {
                    html += `<span class="see-more" onclick="showMore(this, ${chunkId})">${translations[currentLanguage]["see-more"]}</span>`;
                }
                chunkId++;
            }
            return html;
        ...
        function showMore(el, id) {
            const nextChunk = el.parentElement.querySelector(`.chunk-${id + 1}`);
            if (nextChunk) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (item.kind === 'file') {
                    ...
                    if (file) {
                        document.getElementById('upload-loading').classList.remove('hidden');
                        try {
                            await uploadFile(file, file.name || 'pasted_file', file.type);
                        } finally {
                            document.getElementById('upload-loading').classList.add('hidden');
                        }
                    }
